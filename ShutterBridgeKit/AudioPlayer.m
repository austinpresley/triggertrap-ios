...
- (void)setupAudioSession {
    // The trigger tone is played out of the device's audio output (the dongle /
    // headphone route). Use the Playback category WITHOUT DefaultToSpeaker:
    // DefaultToSpeaker is only valid for the PlayAndRecord category and on modern
    // iOS would both fail to apply and force audio to the built-in speaker rather
    // than the connected dongle. Playback also keeps the tone audible when the
    // ring/silent switch is set to silent. Configure the session fully *before*
    // activating it.
    ...
    if (![session setCategory:AVAudioSessionCategoryPlayback error:&error]) {
        NSLog(@"ShutterBridge: failed to set audio category: %@", error);
        error = nil;
    }
    ...
    if (![session setActive:YES error:&error]) {
        NSLog(@"ShutterBridge: failed to activate audio session: %@", error);
    }
}
