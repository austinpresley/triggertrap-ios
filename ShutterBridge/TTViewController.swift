...
class TTViewController: SplitLayoutViewController, DispatchableLifecycle, SequenceLifecycle {
    ...
    fileprivate var notification = MPGNotification(title: NSLocalizedString("Low Volume", comment: "Low Volume"),
                                               subtitle: nil,
                                               backgroundColor: UIColor.shutterBridge_primaryColor(1.0),
                                               iconImage: nil)
    ...
    /** Taptic Engine feedback so the photographer feels each shutter fire. */
    fileprivate let hapticGenerator = UIImpactFeedbackGenerator(style: .medium)
    ...
    override func performThemeUpdate() {
        ...
        self.bulbButton?.setImage(ImageWithColor(UIImage(named: "BulbIcon")!, color: UIColor.shutterBridge_primaryColor()), for: UIControl.State())
    ...
    fileprivate func showVolumeAlert() {
        DispatchQueue.main.async(execute: {
            let alert = UIAlertController(title: NSLocalizedString("Low Volume", comment: "Low Volume"),
                message: NSLocalizedString("Please set the volume to maximum to use ShutterBridge mobile", comment: "Please set the volume to maximum to use ShutterBridge mobile"),
    ...
    fileprivate func showVolumeNotification() {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + Double(Int64(delay * Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC), execute: {
            ...
            self.notification?.backgroundColor = UIColor.shutterBridge_primaryColor()
            self.notification?.titleColor = UIColor.shutterBridge_fillColor()
    ...
    func willDispatch(_ dispatchable: Dispatchable) {
        // Give the photographer tactile confirmation in sync with the shutter
        // pulse, then keep the Taptic Engine warm for the next one.
