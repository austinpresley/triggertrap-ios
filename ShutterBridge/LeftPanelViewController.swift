...
class LeftPanelViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.tableView.backgroundColor = UIColor.shutterBridge_backgroundColor(1.0)
...
extension LeftPanelViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let modes = modes, let modesSection = (modes[indexPath.section] as? NSArray), let modesInSection = modesSection[1] as? NSArray, let mode = modesInSection[indexPath.row] as? NSDictionary {
            ...
            cell.titleLabel.textColor = UIColor.shutterBridge_accentColor()
            cell.identifier = mode["identifier"] as? String
            cell.backgroundColor = UIColor.shutterBridge_fillColor()
            cell.icon.image = ImageWithColor(UIImage(named: mode["icon"] as! String)!, color: UIColor.shutterBridge_iconColor())
            ...
            if WearablesManager.sharedInstance.isWearablesModeRunning() && !cell.wearablesSupported {
                ...
                cell.square.backgroundColor = UIColor.shutterBridge_color(UIColor.shutterBridge_naturalColor(), change: CGFloat(indexPath.row) * 0.1)
                ...
                cell.square.backgroundColor = UIColor.shutterBridge_color(UIColor.shutterBridge_primaryColor(), change: CGFloat(indexPath.row) * 0.1)
            }
            
            cell.descriptionLabel.textColor = UIColor.shutterBridge_foregroundColor()
            cell.separatorView.backgroundColor = UIColor.shutterBridge_clearColor()
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        sectionBackgroundView.backgroundColor = UIColor.shutterBridge_backgroundColor(1.0)
        ...
        label.font = UIFont.shutterBridge_metric_regular(18.0)
        label.textColor = UIColor.shutterBridge_accentColor(1.0)
