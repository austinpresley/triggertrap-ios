...
class TimedReleaseViewController: CableReleaseViewController, TTNumberInputDelegate, TTKeyboardDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let image = ndCalculatorImageView.image {
            ndCalculatorImageView.image = ImageWithColor(image, color: UIColor.shutterBridge_primaryColor())
        }
    ...
    @IBAction func ndCalculatorTapped(_ button: UIButton) {
        ...
        navigationController.navigationBar.barTintColor = UIColor.shutterBridge_primaryColor(1.0)
        
        navigationController.navigationBar.titleTextAttributes = [NSAttributedString.Key.font: UIFont.shutterBridge_metric_regular(23.0), NSAttributedString.Key.foregroundColor: UIColor.shutterBridge_iconColor(1.0)]
