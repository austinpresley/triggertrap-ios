...
class NeutralDensityCalculatorViewController: SplitLayoutViewController, HorizontalPickerDelegate, TTNumberInputDelegate {
    ...
    override func viewDidLoad() {
        ...
        hoursMinutesSecondsView.setFontColor(UIColor.shutterBridge_primaryColor(1.0))
    ...
    @objc func calculateButtonTapped(_ sender: AnyObject) {
        ...
        if duration < UInt64(ConstMinDuration) {
            ShowAlertInViewController(self, title: NSLocalizedString("Slow down, buddy!", comment: "Slow down, buddy!"),  message: NSLocalizedString("This shutter speed is a bit too fast for bulb mode. Try using Shutter Priority on your camera, and ShutterBridge as a cable release.", comment: "This shutter speed is a bit too fast for bulb mode. Try using Shutter Priority on your camera, and ShutterBridge as a cable release."), cancelButton: NSLocalizedString("OK", comment: "OK"))
        } else {
    ...
    override func performThemeUpdate() {
        ...
        filterStrengthLabel.textColor = UIColor.shutterBridge_foregroundColor()
        baseShutterSpeedLabel.textColor = UIColor.shutterBridge_foregroundColor()
        
        applyThemeUpdateToPicker(filterStrengthPicker)
        applyThemeUpdateToPicker(baseShutterSpeedPicker)
        
        shutterSpeedLabel.textColor = UIColor.shutterBridge_primaryColor()
        stopFilterLabel.textColor = UIColor.shutterBridge_primaryColor()
        stepLabel.textColor = UIColor.shutterBridge_primaryColor()
        hoursLabel.textColor = UIColor.shutterBridge_primaryColor()
        
        hoursMinutesSecondsView.setFontColor(UIColor.shutterBridge_primaryColor())
        hoursMinutesSecondsView.borderColor = UIColor.clear
        hoursMinutesSecondsView.borderHighlightColor = UIColor.shutterBridge_primaryColor()
