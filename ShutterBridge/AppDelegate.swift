...
//        generateStringsFromPlists()
        ...
        UINavigationBar.appearance().titleTextAttributes = [NSAttributedString.Key.font: UIFont.shutterBridge_metric_regular(21.0), NSAttributedString.Key.foregroundColor: UIColor.shutterBridge_fillColor(1.0)]
        
        UINavigationBar.appearance().barTintColor = UIColor.shutterBridge_primaryColor()
        
        // Set the window background colour so that we don't see a black flicker when rotating
        window?.backgroundColor = UIColor.shutterBridge_fillColor(1.0)
